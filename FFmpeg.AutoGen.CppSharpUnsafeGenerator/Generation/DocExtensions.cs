...
internal static class DocExtensions
{
    public static void WriteParam(this GeneratorBase generator, ICanGenerateXmlDoc value, string name)
    {
        ...
        if (!string.IsNullOrWhiteSpace(content))
            generator.WriteLine($"/// <param name=\"{name}\">{EscapeXmlString(content)}</param>");
    }

    public static void WriteSummary(this GeneratorBase generator, ICanGenerateXmlDoc xmlDoc)
    {
        string content = xmlDoc.Content?.Trim();
        if (!string.IsNullOrWhiteSpace(content))
            generator.WriteLine($"/// <summary>{EscapeXmlString(content)}</summary>");
    }

    public static void WriteReturnComment(this GeneratorBase generator, FunctionDefinitionBase function)
    {
        string content = function.ReturnComment?.Trim();
        if (!string.IsNullOrWhiteSpace(content))
            generator.WriteLine($"/// <returns>{EscapeXmlString(content)}</returns>");
    }

    public static void WriteObsoletion(this GeneratorBase generator, IObsoletionAware obsoletionAware)
    {
        Obsoletion obsoletion = obsoletionAware.Obsoletion;
        if (obsoletion.IsObsolete)
            generator.WriteLine(string.IsNullOrWhiteSpace(obsoletion.Message) ? "[Obsolete()]" : $"[Obsolete(\"{EscapeQuotes(obsoletion.Message)}\")]");
    }
